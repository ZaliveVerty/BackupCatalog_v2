...
namespace BackupCatalogGUI {
    public partial class UpdateableItemCollectionEditor : Form {
        ...
        private void AddUpdateableItemRow(UpdateableItem item) {
            GridView.Rows.Add(item.DisplayName, item.DestinationFolder);
            DataGridViewRow row = GridView.Rows[GridView.Rows.Count - 1];

            row.SetValues(item.DisplayName);
            row.DisplayImage(NameCol, _imageCache.GetImage(item.GetImageChacheKey(), item.GetIcon(), 22, 22), 27);
        }
        ...
        private void Add() {
            InputResult<IList<object>> selectionResult = ItemSelectorUtilities.ShowSelector(Root, new ItemSelectorProfile() {
                SelectedItemValidator = item => ((Item)item).IsUpdateable() && !UpdateableItems.Contains(item),
